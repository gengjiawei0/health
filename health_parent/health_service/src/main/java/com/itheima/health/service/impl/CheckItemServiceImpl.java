...
public class CheckItemServiceImpl implements CheckItemService {
    ...
    public void update(CheckItem checkitem) {
        ...
        //删除redis中存的套餐的key
        deleteKeyInCheckItem(checkitem.getId());
    }
    ...
    /*
    * 检查项被修改时
    * 可以删除被存入redis的套餐key详情*/
    public void deleteKeyInCheckItem(int id) {
        Jedis jedis = jedisPool.getResource();
        //根据传入id查询关联表
        List<Integer> groupIds = checkItemDao.findGroupIdByItemId(id);
        //如果list不为null
        if (groupIds!=null&&groupIds.size()>0){
            //便利listid
            for (Integer groupId : groupIds) {
                //调用方法查t_setmeal_checkgroup
                List<Integer> setmealIds = checkGroupDao.findSetmealIdByGroupId(groupId);
                //如果关联id集合不为空 删除
